...
def _compute_uncertainty_components(prob_tensor):
    """返回三种不确定性分数：entropy、bvsb、lowest_confidence。"""
...
def run_training(config_path):
    ...
    # 情感分析标签显示映射：0->负类，1->正类
    # 兼容旧任务配置：若未传 classification_scene，但标签全集仅包含 0/1，也执行映射。
    scene_name = str(config.get("classification_scene", "")).strip().lower()
    normalized_label_set = set(
        str(v).strip() for v in output_df['label'].dropna().tolist() if str(v).strip() != ""
    )
    binary_sentiment_tokens = {"0", "1", "0.0", "1.0"}
    should_map_sentiment = (
        scene_name == "sentiment"
        or (normalized_label_set and normalized_label_set.issubset(binary_sentiment_tokens))
    )
    if should_map_sentiment:
        output_df['label'] = output_df['label'].apply(_map_sentiment_label)
