...
def _imread_compat(path):
    """Read image with OpenCV, fallback to imdecode for Windows non-ASCII paths."""
    try:
    ...
    def __init__(self, path, img_size=640, stride=32, auto=True):
        ...
        if path.endswith('.txt'):
            ...
            if '*' in p:
                ...
                files = sorted(glob.glob(os.path.join(p, '**', '*.*'), recursive=True))  # dir (recursive)
...
def extract_boxes(path='../datasets/coco128'):  # from utils.datasets import *; extract_boxes()
    ...
    for im_file in tqdm(files, total=n):
        if im_file.suffix[1:] in IMG_FORMATS:
            # image
            im = _imread_compat(str(im_file))[..., ::-1]  # BGR to RGB
            h, w = im.shape[:2]
