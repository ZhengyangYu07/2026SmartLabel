...
如果你希望在项目平台中直接调用 FlexMatch，请使用我们新增的 `flexmatch_adapter.py`：

- 将你的 PyTorch `model` 实例（未包装 DataParallel）传入 `FlexMatchAdapter`。
- 将平台的 DataLoader 包装为以下格式以供 Adapter 使用：
...
adapter = FlexMatchAdapter(model=my_model, num_classes=NUM_CLASSES, device='cuda')
...
_For ImageNet datasets:_ Please download the ImageNet 2014 dataset (unchanged from 2012) from the official site (link: https://image-net.org/challenges/LSVRC/2012/2012-downloads.php)
Extract the train and the val set into _subfolders_ (the test set is not used), and put them under `train/` and `val/` respectively. Each subfolder will represent a class.
