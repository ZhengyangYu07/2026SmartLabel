    ...
    const TaskManager = {
        ...
        toggleFavorite: async function (taskId, starElement) {
            ...
            try {
                ...
                // 显示操作反馈
                this.showToast(data.message || (isFavorite ? '已收藏任务' : '已取消收藏'),
                ...
                if (isFavorite) {
                    ...
                    if (taskCard && taskList && taskList.firstChild !== taskCard) {
                        // 动画效果：先缩小，再移动，再放大
                        taskCard.style.transition = 'transform 0.3s ease-in-out, opacity 0.3s ease-in-out';
                        ...
                        setTimeout(() => {
                            ...
                            // 恢复动画属性和状态
                            taskCard.style.transition = 'transform 0.2s ease-in-out, box-shadow 0.2s ease-in-out';
        ...
        bindFilterEvents() {
            // 初始化时默认选中“进行中”
            this.filters.selectedStatus = ['processing'];
            ...
            document.querySelectorAll('.status-btn').forEach(btn => {
                btn.addEventListener('click', (e) => {
                    ...
                    if (isActive) {
                        // 移除状态
                        this.filters.selectedStatus = this.filters.selectedStatus.filter(s => s !== status);
                    } else {
                        // 添加状态（去重）
                        this.filters.selectedStatus = [...new Set([...this.filters.selectedStatus, status])];
                    }
            ...
            document.getElementById('search-btn').addEventListener('click', () => {
                ...
                const taskName = document.getElementById('task-name-filter').value.trim();

                // 修正校验逻辑：允许不输入任务名称
                if (!taskType && !taskName && this.filters.selectedStatus.length === 0) {
                    alert('请至少选择一个筛选条件');
                    return;
                }
        ...
        updateButtonStatus() {
            document.querySelectorAll('.status-btn').forEach(btn => {
                ...
                btn.classList.toggle('active', this.filters.selectedStatus.includes(status));
                // 添加样式变化
        ...
        updateFilterStatus() {
            ...
            const activeFilters = this.filters.selectedStatus
                ...
                .concat(
                    this.filters.taskType ? `类型：${this.taskTypes[this.filters.taskType]}` : '',
        ...
        async createTaskCard(task) {
            ...
            const updateTimeHTML = (task.status === 'completed' || task.status === 'checking') && task.updated_at
        ...
        async startProgressPolling(taskId) {
            ...
            const intervalId = setInterval(async () => {
                ...
                if (!elements.fill || !elements.percent || !elements.indicator || !elements.statusText) {
                ...
                try {
                    ...
                    console.log(`[任务 ${taskId}] 状态更新: ${data.progress.toFixed(1)}% (${data.status})`);
        ...
        async updateTaskList() {
            const taskList = document.getElementById('project-list');
            taskList.innerHTML = '<div class="col-span-3 text-center py-8">加载中...</div>';
            ...
            try {
                ...
                if (results.length === 0) {
                    taskList.innerHTML = '<div class="col-span-3 text-center py-8">没有找到相关任务</div>';
                    return;
                }
        ...
        bindMessageListener() {
            window.addEventListener('message', (event) => {
                if (event.data.type === 'favoriteChanged') {
                    ...
                    // 更新当前页面的收藏显示
                    this.updateTaskFavoriteDisplay(event.data.taskId, event.data.isFavorite);
                }
            });
        },
